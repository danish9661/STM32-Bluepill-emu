#!/bin/sh
# Build the bare-metal OTG_FS CDC demo with the Arduino toolchain's gcc.
# No libc/Arduino core (F105 has no Arduino board locally): freestanding
# Cortex-M3, custom linker script. The ELF ships at site/otg_cdc.elf
...
"$GCC" -mcpu=cortex-m3 -mthumb -Os -ffreestanding -nostdlib \
  -Wl,--gc-sections -T "$D/link.ld" "$D/main.c" -o "$D/build/otg_cdc.elf"
cp "$D/build/otg_cdc.elf" "$D/../../site/otg_cdc.elf"
ls -la "$D/../../site/otg_cdc.elf"

...
| I2C1–2 | Full | Master state machine (START, address 7-bit, TX/RX, STOP, repeated START), SR1/SR2 status flags, error handling (AF, BER), interrupts (EV/ER). Slave mode: host-driven via `i2c_inject_start/write/read/stop` (OAR1/OAR2 match + general call, ADDR/STOPF sequences, RXNE/TXE + EV IRQs, stretch-equivalent NACK/None when not ready). 10-bit addressing is **not** implemented (OAR1 mask drops ADDMODE). |

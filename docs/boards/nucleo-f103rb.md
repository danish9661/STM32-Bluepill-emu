...
correctly unmapped; **SVD-map only** = modeled, needs a custom SVD map.
...
| FLASH 128K (program/erase, WRPRTERR) | Yes | Full | option bytes not modeled (no consumer) |
...
| GPIOA–D (+Arduino D0–D51/A0–A8) | Yes | Full | electrical model; aliases in `board_pins.json` (`nucleo_f103rb` key) |

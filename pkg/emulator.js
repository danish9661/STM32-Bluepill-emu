...
function getMUnicorn() {
    // Node.js: load the CJS Emscripten glue directly
    if (typeof process !== 'undefined' && process.versions?.node) {
        return import('module').then(({ createRequire }) => {
            const require = createRequire(import.meta.url);
            try { return require('./unicorn_arm.cjs'); } catch (e) { /* fall through */ }
            return null;
        });
    }
    // Browser: unicorn_arm.js loaded via <script src="unicorn_arm.js"></script>
    if (typeof window !== 'undefined' && window.MUnicorn) return Promise.resolve(window.MUnicorn);
    return Promise.reject(new Error('unicorn_arm module not found. In Node use this package; in browser add <script src="unicorn_arm.js"></script>'));
}

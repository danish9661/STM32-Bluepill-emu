...
2. **Path A — ACCEPTED (branch `path-a`)**: merged single-wasm module (`path-a/merged4.wasm` + `loader-core.mjs` + `emulator-merged.mjs`) passes the full acceptance gate — 236/236 unit tests, 39/39 canary, 200M on both paths, IPS within ~2× baseline, browser page, and the 10-firmware battery `path-a/test_firmware_merged.mjs` 32/32 (spawn-per-section due to the ~1.9GB shared backing per process). Multi-emulator crash fixed via a patched per-instance `mem_map` using `uc_mem_map_ptr` on a shared backing buffer. Docs: docs/NEXT_PHASE.md §4. Dual-wasm stays the shipping default; Path A is a branch.
...
1. ~~**Single WASM module — "Path A"**~~ — **ACCEPTED on branch `path-a` (2026-08-13)**: merged module passes every acceptance gate incl. the 10-firmware battery 32/32. Dual-wasm remains the shipping default (Path A lives on the branch); see docs/NEXT_PHASE.md §4.

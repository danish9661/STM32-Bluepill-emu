...
- **CI** (`.github/workflows/test.yml`): `cmp` guard extended to all three `pkg`↔`site` artifacts (emulator.js, stm32_bluepill_wasm.js, stm32_bluepill_wasm_bg.wasm), stale "37/37" canary comment → 39/39, added the full 200M config run (`echo -n "AB" | node pkg/cli.mjs --config=... --max=200000000`) after the canary. **The wasm guard byte-compares the common prefix** (code sections must match; trailing custom sections + wasm-opt reshaping may differ): wasm-pack 0.14 auto-runs `wasm-opt` on release builds when binaryen is on PATH, so the committed `site/` wasm must come from a build with the SAME binaryen — CI pins binaryen `version_132` (download+PATH step) and `wasm-pack@0.14.0` to match local builds. The guard failed on every master push 2026-08-13 (pre-existing on ee904e6): CI's fresh build differed from the committed artifact; the size guard then also failed, which pinned the cause to the runner's wasm-opt applying a different optimization. Rebuild locally with `PATH=<binaryen-version_132>/bin:$PATH wasm-pack build --target web` and commit the synced site/ copy together with pkg/ changes. The two JS artifacts stay byte-exact.
...
PATH=<binaryen-version_132>/bin:$PATH wasm-pack build --target web   # rebuild pkg (Rust → wasm) — MUST use pinned binaryen version_132 (wasm-opt), else site/ sync breaks the CI guard

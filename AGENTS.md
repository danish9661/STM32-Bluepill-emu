...
- **CI** (`.github/workflows/test.yml`): `cmp` guard extended to all three `pkg`↔`site` artifacts (emulator.js, stm32_bluepill_wasm.js, stm32_bluepill_wasm_bg.wasm), stale "37/37" canary comment → 39/39, added the full 200M config run (`echo -n "AB" | node pkg/cli.mjs --config=... --max=200000000`) after the canary. **wasm guard is size-tolerant** (byte-identical OR same size): the trailing custom sections (producers / wasm-bindgen version stamps) vary with the exact rustc toolchain, and wasm-pack 0.15+ strips 304 of those bytes — the strict byte cmp failed every run once CI drifted to latest wasm-pack (2026-08-13, pre-existing on ee904e6). wasm-pack is pinned to `@0.14.0` to match local builds; the size fallback absorbs rustc@stable drift. The two JS artifacts stay byte-exact.

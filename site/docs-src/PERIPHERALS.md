...
| NVIC | Full | 68 IRQs, priority-based dispatch, pending/active sets, PRIMASK/BASEPRI gating, `last_popped` fairness so hot IRQs don't starve others. ICPR @ 0x180 clears pending; 0x280 writes are RESERVED no-ops (IABR read-only). ISR return is one Rust call (`finish_interrupt(irq)`) that pops the active-priority stack **and** drains SysTick debt ticks internally — no JS re-pend loop; the raw `clear_current_interrupt()` export now also clears the returned entry's IABR bit (the old pop-only return leaked phantom-active IRQs, matching the native `exception_return` path). Driver note: pending bits coalesce intra-batch events (one delivery per batch per IRQ) — rate-accurate runs need production-sized batches (~20K), never 1M steps. Test-driver note: `has_pending_interrupt` consults live INTR_MASK statics (set from CPU PRIMASK per batch), so raw-wasm tests call `set_intr_masks(0,0)` first; `get_next_pending_interrupt` pushes an active-priority entry (HW nesting rules), so multi-take tests return (`clear_current_interrupt`) between takes. |
...
| Board hardware (NRST/BOOT0/LED) | Full | In-WASM board layer (`board_*` exports → `emu.reset()/setBoot0()/boardInfo()`): NRST = model reset + CPU/RAM reload + zeroed counters (BOOT0-high takes the USART1 AN3155 bootloader path); per-board LED + user button identity (Maple BUT=PB8/LOW, Nucleo B1=PC13/HIGH, pills none) + 8MHz/72MHz clock facts. Page NRST/BOOT0/USR widgets, worker + ws-server cases. |
...
| IWDG | Full | Independent watchdog: LSI-clocked free-run down-counter on the batch tick (a started fuse fires on `step_batch` ticks alone — register-access-only decrement never fired it), refresh, `WdogReset{1}` + **emulator stop** on expiry (JS checks `is_watchdog_reset_requested()`). NRST re-anchors the delta base (no catch-up burst). Runs through STOP/STANDBY. |
| WWDG | Full | Window watchdog, same reset semantics (free-run tick arm + NRST rebase, EWI → IRQ0). |
...
| Unit | Level | Notes |
|---|---|---|
| TIM1–7 | Full | PSC/ARR/CNT with instruction-delta advance (no `ticks.min()` cap — ALL accumulated ticks processed per batch), ARR reset = 0xFFFF (RM0008 — the old 0xFFFF_FFFF default made the closed-form jump compute over a 4B-tick window and wedged post-NRST catch-up ticks forever), PWM1/2 output compare (duty exposed via `pwm_duty()`, narrowed by BDTR dead-time on TIM1/TIM8 complementary channels), input capture, update events, UIE/CCIE interrupts, CCR1–4, DMA burst (DCR DBA/DBL window — each DMAR write lands in the next window register and wraps; DCR reprogram restarts). TIM6/7 basic timers included. |

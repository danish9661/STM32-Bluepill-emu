...
| CAN1 | Full | Mailboxes (TIR/TDTR/TDLR/TDHR), filters (ID-list and mask modes), TX request + TX-complete IRQ, RX FIFO + RX IRQ, `can_inject_message()` to inject a received frame from JS. CAN2 (F105 map) is a second instance with its own filter bank (silicon shares banks across CANs — no observable difference for independent filters). |

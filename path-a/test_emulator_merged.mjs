...
// ELF ships in site/ (gitignored in tests/*/build); eeprom/spi_flash images
// are gitignored too — recreate them per the AGENTS.md recipe.
const ELF = new URL('../site/arduino_periph_test.elf', import.meta.url);
const read = () => readFileSync(decodeURIComponent(ELF.pathname));
...
const bin = (f) => readFileSync(join(BIN, f));
...
const emu = await createEmulator({
    ...
    ext_devices: {
        i2c_eeprom: [
            { peripheral: 'I2C1', address: 0x50, data: bin('eeprom.bin') },
            { peripheral: 'I2C2', address: 0x51, data: bin('eeprom2.bin') },
        ],
        i2c_oled: [{ peripheral: 'I2C1', address: 0x3C }],
        spi_flash: [
            { peripheral: 'SPI1', jedec_id: 0xEF4016, data: bin('spi_flash.bin'), cs: 'PA4' },
            { peripheral: 'SPI2', jedec_id: 0xEF4017, data: bin('spi_flash2.bin'), cs: 'PB12' },
        ],

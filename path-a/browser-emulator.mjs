 ...
 */
import { browserLoadMerged as loadMerged } from './browser-loader.mjs';

// Path A: both the peripheral WASM and the Unicorn C core live in ONE merged
// emscripten module (see NEXT_PHASE.md). The rest of this file is the
// unmodified production emulator loop.
let mergedPromise;
function getMerged() {
    if (!mergedPromise) mergedPromise = loadMerged();
    return mergedPromise;
}
function getPeriph() {
    return getMerged().then((m) => m.periph);
}
function getMUnicorn() {
    return getMerged().then((m) => async () => m.Module);
}
